...
def batchnorm_backward(dout, cache):
    ...
    dx = N * dx_hat
    dx -= np.sum(dx_hat, axis=0)
    dx -= x_hat * np.sum(dx_hat * x_hat, axis=0)
    dx /= N * np.sqrt(sample_var + eps)
...
def dropout_forward(x, dropout_param):
    ...
    if mode == 'train':
        ...
        mask = np.ones(x.size) / p
        mask[np.arange((int) (x.size*p))] = 0
