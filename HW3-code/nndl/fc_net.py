...
class TwoLayerNet(object):
  ...
  def loss(self, X, y=None):
    ...
    scores, cache_h2 = affine_forward(h1_relu, self.params['W2'], self.params['b2'])
    ...
    l2reg = 0.5 * self.reg * (np.sum(self.params['W1'] ** 2) + np.sum(self.params['W2'] ** 2))

    loss, dscores = softmax_loss(scores, y)
    loss += l2reg

    dx2, dw2, db2 = affine_backward(dscores, cache_h2)
    grads['W2'] = dw2 + self.reg * self.params['W2']
    grads['b2'] = db2

    dx1_relu = relu_backward(dx2, cache_h1_relu)
    dx1, dw1, db1 = affine_backward(dx1_relu, cache_h1)
...
class FullyConnectedNet(object):
  ...
  def loss(self, X, y=None):
    ...
    for n in range(self.num_layers - 1):
        h, c = affine_forward(h, self.params['W' + str(n+1)], self.params['b' + str(n+1)])
        caches['cache' + str(n+1)] = c
        h, c = relu_forward(h)
        caches['cache_relu' + str(n+1)] = c
    ...
    for n in range(self.num_layers-1, 0, -1):
        relu_grad = relu_backward(upstream_grad, caches['cache_relu' + str(n)])
        dx, dw, db = affine_backward(relu_grad, caches['cache' + str(n)])
        upstream_grad = dx
