...
class TwoLayerNet(object):
  ...
  def loss(self, X, y=None, reg=0.0):
    ...
    h1 = (W1.dot(X.T) + np.reshape(b1, (b1.shape[0], 1))).T
    h1_relu = np.maximum(h1, 0, h1)
    scores = h1_relu.dot(W2.T) + np.reshape(b2, (1, b2.shape[0]))
    ...
    regularizer = 0.5 * reg * (np.sum(W1 ** 2) + np.sum(W2 ** 2))
    loss += regularizer
    # ================================================================ #
    # END YOUR CODE HERE
    # ================================================================ #
    ...
    gscores = probs
    gscores[np.arange(N),y] -= 1
    gscores /= N

    dl_dW2 = np.dot(gscores.T, h1_relu)
    dl_dW2 += reg * W2
    dl_db2 = np.sum(gscores, axis=0, keepdims=True).flatten()

    dl_drelu = np.dot(gscores, W2)
    dl_drelu[h1 <= 0] = 0

    dl_dW1 = np.dot(X.T, dl_drelu).T
    dl_dW1 += reg * W1
    
    dl_db1 = np.sum(dl_drelu, axis=0, keepdims=True).flatten()
  ...
  def train(self, X, y, X_val, y_val,
    ...
    for it in np.arange(num_iters):
      ...
      # YOUR CODE HERE:
  	  ...
  	  # ================================================================ #
      idx = np.random.choice(np.arange(num_train), min(batch_size, num_train))
      X_batch = X[idx]
      y_batch = y[idx]
