...
def rmsprop(w, dw, config=None):
  ...
  next_w = w - (config['learning_rate'] / (np.sqrt(a) + config['epsilon'])) * dw
  config['a'] = a
...
def adam(w, dw, config=None):
  ...
  v = config['beta1'] * config['v'] + (1 - config['beta1']) * dw
  a = config['beta2'] * config['a'] + (1 - config['beta2']) * dw * dw
  v_ = (1 / (1 - config['beta1'] ** config['t'])) * v
  a_ = (1 / (1 - config['beta2'] ** config['t'])) * a
  next_w = w - (config['learning_rate'] / (np.sqrt(a_) + config['epsilon'])) * v_
